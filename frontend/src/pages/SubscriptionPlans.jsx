...
export default function SubscriptionPlans({ user }) {
  ...
  return (
    <div className="-mx-4 sm:-mx-6 lg:-mx-8 -my-8 bg-surface min-h-screen">
      <main className="max-w-7xl mx-auto px-6 md:px-8 pt-32 pb-24">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start mb-28 pt-12 px-2">
            {plans.map((plan, idx) => {
        ...
        {/* ============================================================
            4. VISUAL CTA ANCHOR
            ============================================================ */}
        <section className="mt-24 relative min-h-[400px] py-16 px-6 rounded-[2rem] overflow-hidden bg-on-secondary-fixed">
...
function PlanCard({ plan, isPopular, isCurrent, billing, onSubscribe, loading }) {
  ...
  return (
    <div className={[
      'relative',
      isPopular ? 'z-10 lg:scale-[1.03] pt-5' : 'pt-5',
    ].join(' ')}>
      {isPopular && (
        <div className="absolute top-0 left-1/2 -translate-x-1/2 z-10 bg-gradient-to-r from-primary to-primary-container text-white text-[10px] font-black px-5 py-1.5 rounded-full tracking-widest uppercase shadow-ambient whitespace-nowrap">
          Most Popular
        </div>
      )}
    <Card
      padding="lg"
      className={[
        'flex flex-col rounded-3xl transition-all h-full',
        isPopular
          ? 'shadow-ambient ring-2 ring-primary/30'
          : '',
      ].join(' ')}
    >
      {/* Plan header */}
