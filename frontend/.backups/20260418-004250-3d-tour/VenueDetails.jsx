...
export default function VenueDetails({ user }) {
  ...
  const minPrice = tables.length > 0
    ? Math.min(...tables.map((t) => t.price_per_hour || 0).filter(Boolean))
    : 0;
  const pricePerGuest = minPrice || (priceRange * 30);
  ...
  return (
    <div className="-mx-4 sm:-mx-6 lg:-mx-8 -mt-8 bg-surface">
      {/* ============================================================
          1. HERO GALLERY
          ============================================================ */}
      <section className="relative w-full h-[480px] md:h-[600px] overflow-hidden bg-on-secondary-fixed group">
        {heroImg ? (
          <img
            key={heroIdx}
            src={heroImg}
        ...
        <div className="flex-1 min-w-0 space-y-12">
          ...
          {tab === 'overview' && (
            <section className="space-y-10">
              ...
              <div className="relative w-full aspect-video bg-on-secondary-fixed rounded-2xl overflow-hidden flex flex-col items-center justify-center text-white/60 group cursor-grab active:cursor-grabbing">
                <div className="absolute inset-0 bg-gradient-to-tr from-on-secondary-fixed via-on-secondary-fixed to-primary-container/30 opacity-70" />
                <View className="h-16 w-16 mb-4 group-hover:scale-110 transition-transform relative" strokeWidth={1.2} />
                <h4 className="text-white text-xl font-semibold relative">Interactive 3D View</h4>
                <p className="text-sm font-medium opacity-70 relative">Click and drag to orbit</p>
                <div className="absolute bottom-6 right-6 flex gap-2">
                  <GlassIconButton aria-label="Zoom in"><ZoomIn className="h-4 w-4" /></GlassIconButton>
                  <GlassIconButton aria-label="Fullscreen"><Maximize className="h-4 w-4" /></GlassIconButton>
                </div>
              </div>
          ...
          <div className="sticky top-24 space-y-6">
            <Card padding="lg" className="rounded-[2rem] shadow-ambient">
              <div className="flex justify-between items-center mb-7">
                <div>
                  <span className="text-3xl font-bold text-on-surface">${pricePerGuest}</span>
                  <span className="text-on-surface-variant text-sm font-medium"> / guest</span>
                </div>
